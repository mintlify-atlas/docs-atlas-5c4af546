...
<CodeGroup>
```bash cURL
curl -X GET "https://api.aero.com/v1/airlines?search=United" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.aero.com/v1/airlines?search=United', {
  headers: {
...
response = requests.get(
    'https://api.aero.com/v1/airlines',
...
```bash
curl -X GET "https://api.aero.com/v1/airlines?search=Delta" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X GET "https://api.aero.com/v1/airlines?search=AA" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X GET "https://api.aero.com/v1/airlines?search=AAL" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X GET "https://api.aero.com/v1/airlines?search=South" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
const searchAirlines = async (query) => {
  const response = await fetch(
    `https://api.aero.com/v1/airlines?search=${encodeURIComponent(query)}`,
    {
      headers: { 'Authorization': 'Bearer YOUR_JWT_TOKEN' }
    }
  );
...
const getAirlineByCode = async (iataCode) => {
  const response = await fetch(
    `https://api.aero.com/v1/airlines?search=${iataCode}`,
    {
      headers: { 'Authorization': 'Bearer YOUR_JWT_TOKEN' }
    }
  );
