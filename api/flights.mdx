...
const response = await fetch('http://localhost:3000/v1/flights?from=LAX&to=JFK&date=2024-03-20', {
  headers: {
...
```javascript JavaScript
const response = await fetch('http://localhost:3000/v1/flights/airline', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_JWT_TOKEN',
...
response = requests.post(
    'http://localhost:3000/v1/flights/airline',
    json={'search': '6E9920'},
    headers={'Authorization': 'Bearer YOUR_JWT_TOKEN'}
)
data = response.json()
...
```javascript JavaScript
const response = await fetch('http://localhost:3000/v1/flights/tracked', {
  headers: {
    'Authorization': 'Bearer YOUR_JWT_TOKEN'
  }
});
